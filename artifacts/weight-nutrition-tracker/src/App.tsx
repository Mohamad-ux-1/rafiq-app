...
function MobileNav() {
  ...
  return (
    <nav
      className="fixed bottom-4 left-4 right-4 z-30 flex items-center justify-around rounded-2xl border bg-[hsl(var(--card)/.92)] px-2 py-2.5 shadow-2xl backdrop-blur-md md:hidden"
      dir="rtl"
    >
      {navItems.map(({ href, label, icon: Icon }) => (
        <Link
          ...
          ({ name, tag, calories, unit, note, icon: Icon }, index) => (
            ...
            >
              <div className="mb-5 flex items-start justify-between">
                ...
                <button
                  onClick={() => toggleSavedGuideItem(name)}
...
function SavedPage() {
  const { data, toggleSavedGuideItem } = useRafiq();
  const savedItems = guideFoods.filter((f) =>
  ...
  return (
    <div className="mx-auto max-w-[1160px] p-4 sm:p-6 md:p-9 lg:p-12" dir="rtl">
      <PageHeader
        eyebrow="أطباقك المفضلة"
        title="المحفوظات"
        description="الأطعمة والنصائح التي احتفظت بها للرجوع إليها بسرعة."
      />
      {savedItems.length === 0 ? (
          ...
          {savedItems.map(({ name, tag, calories, unit, note, icon: Icon }) => (
            <article key={name} className="card-surface rounded-[22px] p-5">
              <div className="mb-5 flex items-start justify-between">
                ...
                <button
                  onClick={() => toggleSavedGuideItem(name)}
                  className="rounded-full p-2 bg-[hsl(var(--accent)/.25)] text-[hsl(var(--accent-foreground))] transition-colors hover:opacity-80"
                >
        ...
        {sortedLogs.length === 0 ? (
          ...
          sortedLogs.map((log) => {
            ...
            return (
              <div
                key={log.date}
                className="card-surface flex flex-col gap-4 rounded-[24px] p-5"
              >
                <div className="flex items-center justify-between border-b border-[hsl(var(--border)/.6)] pb-3">
                  ...
                  {weightThatDay && (
                    <div className="rounded-full bg-[hsl(var(--secondary))] px-3 py-1 text-xs font-bold text-[hsl(var(--primary))]">
                      وزنك: {weightThatDay} كغ
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div>
                    <div className="mb-2 text-[11px] font-bold text-[hsl(var(--muted-foreground))]">
                      العادات المنجزة ({habitsDone}/4)
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {habits.map((h) => {
                        ...
                        return (
                          <div
                            key={h.key}
                            className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold transition-colors ${done ? "bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))]" : "bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground)/.5)]"}`}
                          >
                            <h.icon size={12} strokeWidth={done ? 2.5 : 1.5} />{" "}
                            {h.title}
                          </div>
                        );
                      })}
